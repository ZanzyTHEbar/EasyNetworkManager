...
APIServer::APIServer(int CONTROL_PORT, ProjectConfig *configManager, const std::string &api_url,
                     const std::string &wifimanager_url, const std::string &userCommands)
    : BaseAPI(CONTROL_PORT, configManager, api_url, wifimanager_url, userCommands) {}
...
void APIServer::begin() {
    ...
    server.on(buffer, HTTP_ANY, [&](AsyncWebServerRequest *request) { handleRequest(request); });

    char buf[1000];
    snprintf(buf, sizeof(buf), "^\\%s\\/([a-zA-Z0-9]+)\\/command\\/([a-zA-Z0-9]+)$", this->wifimanager_url.c_str());
    server.on(buf, HTTP_ANY, [&](AsyncWebServerRequest *request) { handleRequest(request); });
    server.begin();
}
 ...
 * @warning  \c This function requries the user to access the index using a url parameter \c we need to fix this!! I
 * need a better implemenation
 *
 */
